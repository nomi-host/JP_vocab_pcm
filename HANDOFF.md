...
#### v1.1.0 — 실기기 검증 완료 + 운영 반영(정식 배포)

- v1.0.2~v1.0.23의 기능 개선(단어상세 스와이프 닫기, 가이드 재열람+신규 항목·아이콘·문구, 업데이트 안내 팝업, 온보딩 카테고리 설명 정렬, 그리고 **모달 딤을 헤더 아래부터 그라데이션 처리해 상태바/헤더 경계선을 없앤 것**, 모달·팝업 위치 정리)을 실기기(iOS standalone PWA)에서 확인 완료 → `main`에 머지해 정식 배포.
- 상태바 딤 문제 최종 해법: 상태바를 억지로 어둡게 맞추는 게 아니라(그 방향은 v1.0.5~14에서 전부 실패·부작용), **상단(상태바+헤더)은 딤하지 않고 딤을 헤더 아래부터 위→아래 그라데이션(위쪽 opacity 0)으로** 줘서 경계선을 없앰. (오리지날 개인 앱 방식). 상세는 handoff_for_original.md 섹션 12 참고.
- APP_VERSION `v1.1.0` / APP_BUILD `2026-07-02`.
...
| 환경 | URL | 상태 |
|---|---|---|
| 운영(Production) | pitto-voca.vercel.app | `main` 브랜치, **v1.1.0까지 반영** |
| 프리뷰(Preview) | `claude/app-structure-handoff-53n20j` 브랜치 Vercel 프리뷰 URL | v1.1.0 반영 |

...
{bgDetailCard && (
  <DetailModal card={bgDetailCard} allCards={cards} cards={cards}
    voiceRef={voiceRef} onClose={() => setBgDetailCard(null)} />
)}
```
> `ListenView`는 `setCards`를 받지 않는 컴포넌트라(흘려듣기는 읽기 전용 화면) `onToggleStar`/
> `onTogglePriority`/`onToggleDeck`을 일부러 안 넘겼습니다 — 즉 흘려듣기 중 연 상세에는
> 액션 버튼이 안 뜨고 단어/한자/예문 탐색만 됩니다. 액션까지 넣고 싶다면 `App`에서
> `setCards` 기반 토글 함수들을 `ListenView`까지 props로 내려야 합니다(범위가 커짐).
